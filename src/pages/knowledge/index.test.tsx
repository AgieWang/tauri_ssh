...
describe("KnowledgePage", () => {
  ...
  it("知识来源以中文展示类型，并将多选 Git 工作区分别保存", async () => {
    const user = userEvent.setup();
    gitWorkspaceApi.list.mockResolvedValue([
      {
        id: 1,
    ...
    await screen.findByText("团队知识库");
    await user.click(screen.getByRole("tab", { name: "来源与同步" }));
    await user.click(screen.getByRole("button", { name: "添加来源" }));
    const dialog = screen.getByRole("dialog");

    await user.click(within(dialog).getByLabelText("版本策略"));
    expect((await screen.findAllByText("未版本化")).length).toBeGreaterThan(1);
    expect(screen.getByText("按 Git 引用")).toBeInTheDocument();
    await user.click(within(dialog).getByLabelText("同步模式"));
    expect((await screen.findAllByText("增量同步")).length).toBeGreaterThan(1);
    expect(screen.getByText("手动同步")).toBeInTheDocument();

    await user.click(within(dialog).getByLabelText("来源类型"));
    await user.click(
    ...
    fireEvent.change(within(dialog).getByLabelText("来源标识"), {
      target: { value: "team-repositories" },
    });
    ...
    await waitFor(() => {
      expect(knowledgeApi.upsertSourcesAtomically).toHaveBeenCalledWith([
        expect.objectContaining({
          sourceKey: "team-repositories",
          displayName: "团队仓库",
          gitWorkspaceKey: "fj-workorder",
        }),
        expect.objectContaining({
          sourceKey: "team-repositories-tauri-ssh",
          displayName: "团队仓库 · tauri-ssh",
          gitWorkspaceKey: "tauri-ssh",
        }),
      ]);
    });
  });
  ...
  it("来源列表以中文展示类型和最近同步状态", async () => {
    const user = userEvent.setup();
    knowledgeApi.listSources.mockResolvedValue([
      {
        id: 1,
        sourceKey: "team-repository",
        projectId: null,
        sourceType: "git_workspace",
        displayName: "团队 Git 工作区",
        rootPath: "",
        gitWorkspaceKey: "team-repository",
        includeGlobs: [],
        excludeGlobs: [],
        ...
        lastSyncedAt: "2026-08-01 17:28:14",
        lastError: null,
        createdAt: "2026-08-01 17:00:00",
        updatedAt: "2026-08-01 17:28:14",
        deletedAt: null,
      },
    ]);
    renderPage();
    ...
    expect(await screen.findByText("Git 工作区")).toBeInTheDocument();
    expect(screen.getByText("同步成功")).toBeInTheDocument();
    expect(screen.queryByText("git_workspace")).not.toBeInTheDocument();
  ...
  it("同步任务显示持久化进度", async () => {
    ...
    expect(screen.getByText("正在读取受控范围")).toBeVisible();
  });
  ...
  it("活动同步任务自动刷新任务和来源状态", async () => {
    knowledgeApi.listSources.mockReset();
    knowledgeApi.listJobs.mockReset();
    const source = {
      id: 1,
      sourceKey: "team-repository",
      projectId: null,
      ...
      lastError: null,
      createdAt: "2026-08-01 17:00:00",
      updatedAt: "2026-08-01 17:00:00",
      deletedAt: null,
    };
    knowledgeApi.listSources
      .mockResolvedValueOnce([{ ...source, lastSyncStatus: "running" }])
      .mockResolvedValue([{ ...source, lastSyncStatus: "success" }]);
    ...
    await waitFor(
      () => {
        expect(knowledgeApi.listJobs.mock.calls.length).toBeGreaterThanOrEqual(
          2,
        );
      },
      { timeout: 2_500 },
    );
    fireEvent.click(screen.getByRole("tab", { name: /来源与同步/ }));
    expect(await screen.findByText("已完成")).toBeVisible();
    expect(screen.getByText("同步成功")).toBeVisible();
  });

  it("无变更的已完成同步不将零进度显示为未知", async () => {
    const user = userEvent.setup();
    knowledgeApi.listJobs.mockResolvedValue([
      {
        jobKey: "job-no-change",
        jobType: "source_sync",
        status: "completed",
        progressCurrent: 0,
        progressTotal: 0,
    ...
    expect(await screen.findByText("无变更")).toBeVisible();
