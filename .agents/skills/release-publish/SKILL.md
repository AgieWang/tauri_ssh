...
> 构建完成后，用户手动从 GitHub Release 下载产物，Codex 负责本地处理和推送。

### 平台配置

发布流程支持按需选择构建平台，通过 `.Codex/release-config.json` 的 `platforms` 字段配置：
...
支持可选的 R2 CDN 作为主下载源，通过 `.Codex/release-config.json` 的 `r2.enabled` 字段控制：
...
因此改为用户本地下载产物后，由 Codex 在本地完成推送，速度更快且更可控。
...
根据 `.Codex/release-config.json` 中的 `platforms` 配置，修改 `.github/workflows/release.yml` 的构建矩阵：
...
### 步骤 4：自动监听 CI + 自动下载产物（Codex 全自动，无需用户介入）

> **演化背景**：早期流程让用户在浏览器手动下载所有 CI 产物，再 `AskUserQuestion` 询问目录。
> 实操中用户经常下漏文件、下到默认目录被多项目混在一起、下到一半浏览器中断。
> 现升级为 **Codex 自己用 git credential helper 拿 token → API 轮询 CI → 用 asset id 下载到独立子目录**。
...
### 步骤 5：处理下载的产物 + 更新 README（Codex 自动执行）

用户提供下载目录后，Codex 自动执行以下操作：
...
- 用户不需要在本地做任何签名操作
- Codex 读取 `.sig` 文件内容来生成 `update.json`
...
> **Windows 环境变量设置注意事项**
>
> Codex 的 Bash 工具运行在 Git Bash (MSYS2) 环境中。
