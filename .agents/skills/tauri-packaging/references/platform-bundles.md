...
1. NSIS 安装语言
2. 构建命令与 bundle 配置
3. Windows、macOS、Linux 差异
4. 图标、体积和输出目录
5. 版本、CI 与常见错误

## NSIS 安装语言的条件规则

`languages: ["SimpChinese"]` 配合 `displayLanguageSelector: false` 会让英文 Windows 也显示中文安装界面。仅当产品明确要求“安装器始终中文”时使用；它不是通用默认。

需要中英文由用户选择时，配置两个语言并打开选择器；需要跟随系统语言时，按当前 Tauri/NSIS 配置与产品验收结果设置。语言策略改变后必须至少在中文和英文 Windows 环境验证安装、卸载与升级界面。
...
示例只代表单一 Windows 目标。实际 `targets` 从本次明确的平台/产物要求选择；不要把 `"all"` 作为 CI 通用默认。
