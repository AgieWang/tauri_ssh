...
> **核心原则**：`.Codex/skills/` 是主目录（source of truth），`.codex/skills/` 是镜像。任何技能的新增或修改，都必须同步到两个目录。

---

## Codex Commands 与 Codex Skills 的对应关系

> Codex **没有** commands 系统。Codex 的每个 command（`.Codex/commands/xxx.md`）必须在 Codex 中以 skill（`.codex/skills/xxx/SKILL.md`）的形式存在。

**核心区别**：
- **Skill**：两端文件完全相同（直接复制）
- **Command**：Codex 是纯 Markdown（无 YAML 头），Codex 对应的 skill 需要**加 YAML 头部** + 相同正文

**同步规则**：
- 新增/修改/删除 Codex command 时，必须同步 `.codex/skills/` 下对应的 skill
...
### 3.1 在 Hook 中声明（`.Codex/hooks/skill-forced-eval.cjs`）
...
**修改步骤**：
1. 打开 `.Codex/hooks/skill-forced-eval.cjs`
...
```bash
# 检查 hook 文件
grep "add-skill" .Codex/hooks/skill-forced-eval.cjs
...
新增 Codex command 时，除了创建 `.Codex/commands/xxx.md`，还需要：
...
由于框架支持 Codex 和 Codex 两个系统，需要保持技能同步。
...
2. **复制文件**：
   将 `.Codex/skills/[技能名]/SKILL.md` 复制到 `.codex/skills/[技能名]/SKILL.md`
...
**示例**（tauri-commands）：
```
.Codex/skills/tauri-commands/SKILL.md (655 行)
.codex/skills/tauri-commands/SKILL.md  (655 行，完全相同)
```

### 4.2 Command 同步到 Codex

新增/修改 Codex command 时，同步 `.codex/skills/[command名]/SKILL.md`（加 YAML 头部 + 相同正文）。

### 4.3 检查清单

- [ ] `.codex/skills/` 下对应目录已创建
- [ ] Skill：两端文件内容完全相同
- [ ] Command：Codex 文件包含 YAML 头部 + 与 Codex command 一致的正文
...
grep -n "[技能名]:" .Codex/hooks/skill-forced-eval.cjs
...
创建文件：`.Codex/skills/tauri-commands/SKILL.md`
...
**在 hook 中添加**（`.Codex/hooks/skill-forced-eval.cjs`）：
```javascript
...
复制文件：
```
.codex/skills/tauri-commands/SKILL.md  (655 行，与 .Codex 完全相同)
```
...
> **核心原则**：`.Codex/skills/` 是主目录（source of truth），`.codex/skills/` 是镜像。
> 所有修改都在 `.Codex/skills/` 中进行，然后同步到 `.codex/skills/`。
...
1. **在 `.Codex/skills/[技能名]/SKILL.md` 中修改内容**
2. **同步到 Codex**：
   ```bash
   cp .Codex/skills/[技能名]/SKILL.md .codex/skills/[技能名]/SKILL.md
   ```
3. **验证一致性**：
   ```bash
   diff .Codex/skills/[技能名]/SKILL.md .codex/skills/[技能名]/SKILL.md
   ```
   输出应为空（无差异）
...
2. **同步修改 Hook 文件**（`.Codex/hooks/skill-forced-eval.cjs`）中对应的触发词行
...
5. **验证三处一致**：
   ...
   diff .Codex/skills/[技能名]/SKILL.md .codex/skills/[技能名]/SKILL.md
   # 验证 Hook 声明
   grep "[技能名]" .Codex/hooks/skill-forced-eval.cjs
   # 验证 AGENTS.md 声明
   grep "[技能名]" AGENTS.md
   ```
...
1. **创建新目录并移动文件**：
   ...
   mv .Codex/skills/[旧名称]/SKILL.md .Codex/skills/[新名称]/SKILL.md
   rmdir .Codex/skills/[旧名称]

   # Codex
   mkdir -p .codex/skills/[新名称]
   cp .Codex/skills/[新名称]/SKILL.md .codex/skills/[新名称]/SKILL.md
   rm -rf .codex/skills/[旧名称]
   ```
2. **修改 SKILL.md 中 YAML 头部的 `name` 字段**
3. **修改 Hook 文件**（`.Codex/hooks/skill-forced-eval.cjs`）中的技能名
4. **修改 AGENTS.md** 中的技能名
5. **全局搜索旧名称**，确保无遗漏引用：
   ```bash
   grep -r "[旧名称]" .Codex/ .codex/ AGENTS.md AGENTS.md
   ```
...
### 场景 5：修改 Codex Command

修改 `.Codex/commands/xxx.md` 后，同步正文到 `.codex/skills/xxx/SKILL.md`（保留其 YAML 头部）。

### 修改后的检查清单

每次修改现有技能后，必须通过以下检查：

- [ ] `.Codex/skills/[技能名]/SKILL.md` 已修改
...
**解决**：
- 始终修改 `.Codex/hooks/skill-forced-eval.cjs`（注意 `.cjs` 扩展名）
- 同理，`pre-tool-use.cjs` 也是 `.cjs` 扩展名

### 陷阱 3：忘记复制到 Codex 目录

**解决**：`cp -r .Codex/skills/[技能名] .codex/skills/`
...
### 声明阶段
- [ ] Hook 文件已更新（`.Codex/hooks/skill-forced-eval.cjs`）
- [ ] AGENTS.md 已更新（技能表格）
- [ ] 两处声明的触发词一致
...
touch .Codex/skills/[技能名]/SKILL.md
