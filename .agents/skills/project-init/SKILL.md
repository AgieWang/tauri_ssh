...
> - 该规则继承自全局 AGENTS.md 的"仓库创建规范"章节
>
...
├── 2.4 替换作者和描述
├── 2.5 配置更新地址和签名
├── 2.6 更新框架文档中的引用（AGENTS.md / AGENTS.md / commands/）
├── 2.7 配置唯一开发端口号
└── 2.8 验证替换结果
...
> 🔴 **仓库可见性强制规则**：
> - **源码 / 主仓库**：**必须私有**（`private: true`），绝对禁止公开。继承自全局 AGENTS.md。
...
| 是否排除 .Codex/projects/ | 自动排除（未跟踪） | 需手动排除 |
...
| `.Codex/projects/` | 个人 memory 数据，未跟踪 |
| `dist/` | 构建产物，未跟踪 |
...
| 保留项 | 后续处理 |
|--------|---------|
| `.Codex/` （除 `projects/` 未跟踪外） | 技能、命令、hook、配置全部保留；Step 2.2 会更新命令模板里的标识符引用 |
| `.codex/` | Codex 技能镜像，仅用 Codex 的项目可在 Step 1.3 中删除 |
| `AGENTS.md` / `AGENTS.md` | 项目规范文档，Step 2.2 需替换其中的 `com.agilefr.tauri` |
...
```bash
# 3. 如果只使用 Codex，不使用 Codex，可删除 Codex 镜像
# rm -rf .codex
...
ASSET_DIRS=(
  ...
  ".Codex/docs/brainstorm" # 头脑风暴会话沉淀
)
...
| 把 `.Codex/skills/` 当资产剪切 | 严禁——技能必须留在模板 |
...
### Step 1.5.A：写入 `.Codex/exp.config.json`（subproject 模式标记）

在新项目根目录的 `.Codex/` 下创建：

```bash
cat > "$NEW_DIR/.Codex/exp.config.json" <<EOF
{
  "mode": "subproject",
  ...
  "experienceLoop": {
    ...
    "feedbackChannel": ".Codex/docs/experience/feedback-to-framework.md"
  }
}
EOF
```
...
### Step 1.5.B：写入 `.Codex/PROJECT.md`（项目专属约束）

新项目的 `.Codex/PROJECT.md` 是与 `AGENTS.md` 互补的"可写副本"：

```bash
cat > "$NEW_DIR/.Codex/PROJECT.md" <<EOF
...
> 真正通用、可反哺到框架的经验请写到 \`.Codex/docs/experience/feedback-to-framework.md\`。
...
### Step 1.5.C：创建 `.Codex/project-skills/` 目录

项目专属技能存放点（与框架本体 `.Codex/skills/` 区分）：

```bash
mkdir -p "$NEW_DIR/.Codex/project-skills"
cat > "$NEW_DIR/.Codex/project-skills/README.md" <<EOF
...
与 \`.Codex/skills/\` 完全一致：每个技能一个目录，含 \`SKILL.md\`（YAML frontmatter + 内容）。

Codex 会**同时**扫描两个目录，project-skills 中的技能会自动可用。
EOF
```

---

### Step 1.5.D：创建 `.Codex/docs/experience/feedback-to-framework.md`

反哺候选汇集池：

```bash
mkdir -p "$NEW_DIR/.Codex/docs/experience"
cat > "$NEW_DIR/.Codex/docs/experience/feedback-to-framework.md" <<EOF
# 反哺到 tauri 框架的经验候选
...
6. **建议反哺位置** — 框架里哪个 skill / AGENTS.md 章节
...
### Step 1.5.E：在子项目 AGENTS.md 顶部加 PROJECT.md 加载提示

子项目的 `AGENTS.md`（从框架复制来的副本）顶部插入一段，提示读 PROJECT.md：

```bash
# 检查是否已加载提示（避免重复插入）
if ! grep -q "PROJECT.md（项目专属约束）" "$NEW_DIR/AGENTS.md"; then
  # 用 Python 在 "# AGENTS.md" 第一行后插入
  python -c "
import io
p = r'$NEW_DIR/AGENTS.md'
with open(p, 'r', encoding='utf-8') as f:
    lines = f.readlines()
insert_at = next((i+1 for i, l in enumerate(lines) if l.startswith('# AGENTS.md')), 1)
snippet = '''
> 🔴 **子项目模式提示**：本项目是基于 [tauri 框架](https://gitcode.com/zhuawashi/tauri) 创建的子项目。
>
> - 本文件（AGENTS.md）= 框架只读副本，不要直接改
> - 项目专属约束请写到 \`\`.Codex/PROJECT.md\`\`（项目专属约束）
...
| # | 操作 | 产出 |
|---|------|------|
| 1.5.A | 写 exp.config.json | `.Codex/exp.config.json` |
| 1.5.B | 写 PROJECT.md | `.Codex/PROJECT.md` |
| 1.5.C | 建 project-skills/ + README | `.Codex/project-skills/README.md` |
| 1.5.D | 建 feedback-to-framework.md | `.Codex/docs/experience/feedback-to-framework.md` |
| 1.5.E | AGENTS.md 顶部加提示 | 子项目 AGENTS.md 已注入 subproject 提示 |
...
| `AGENTS.md` | `com.agilefr.tauri` → `{新标识符}` | 文档中的引用 |
| `AGENTS.md` | `com.agilefr.tauri` → `{新标识符}` | Codex 项目规范（与 AGENTS.md 对应） |
| `.Codex/commands/progress.md` | `com.agilefr.tauri` → `{新标识符}` | 进度报告模板 |
| `.Codex/commands/start.md` | `com.agilefr.tauri` → `{新标识符}` | 项目介绍模板 |

> **注意**：`.Codex/skills/` 中的技能文档如果包含 `com.agilefr.tauri` 作为示例引用，**不需要替换**。
...
| 文件 | 需要更新的内容 |
|------|--------------|
| `AGENTS.md` | 应用标识 `com.agilefr.tauri` → 新标识符 |
| `AGENTS.md` | 应用标识 `com.agilefr.tauri` → 新标识符（Codex 侧项目规范） |
| `.Codex/commands/progress.md` | 应用标识引用 |
| `.Codex/commands/start.md` | 应用标识引用 |

> **注意**：AGENTS.md / AGENTS.md 中大量内容是通用的架构文档，只需要替换具体的标识符值，不要改动架构说明。
>
...
**允许残留的位置**（不需要替换）：
- `.Codex/skills/*/SKILL.md` — 技能文档中的示例引用
...
> **绝对禁止**创建为 `false`（公开仓库）。即使用户未明确要求，默认也必须是私有。该规则遵循全局 AGENTS.md 的"仓库创建规范"。
...
> **目的**：用户切换到新项目后，一句话即可让 Codex 进入自主连续开发。本步骤输出 `/loop /dev-loop` 起飞指令——配合已剪切进来的需求+原型，新项目无需再手写开发提示词。
...
```bash
# 保存到 .Codex/docs/init-prompt.md，便于用户随时再用
mkdir -p "$NEW_DIR/.Codex/docs"
cat > "$NEW_DIR/.Codex/docs/init-prompt.md" << 'EOF'
# 初始化提示词（项目首次开发用）
...
echo ""
echo "已保存到：$NEW_DIR/.Codex/docs/init-prompt.md"
echo "下次再用直接：cat .Codex/docs/init-prompt.md | xclip"
```
...
| 新项目没继承 `dev-loop` 命令就让它跑 | 确认 `.Codex/commands/dev-loop.md` 已随骨架继承（缺则补） |
| 不保存起飞指令 | 写入 `.Codex/docs/init-prompt.md` 备用 |
