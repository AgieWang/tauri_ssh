...
**与 Codex 内置 TaskCreate 的区别**：内置工具仅**会话内**有效，对话结束即丢；本技能把关键上下文**落到文件**，跨窗口/跨天也能续跑。
...
| 简单修改一两个文件就完事 | ❌ | 用 Codex 内置 TaskCreate 即可 |
| 纯咨询问答 | ❌ | 不涉及开发行为 |
...
#    不要用 sed（跨平台行为不一致），用 Codex 的 Edit 工具
...
💡 建议：项目已用方案 A（见 AGENTS.md 的"绝对禁止"表），保持一致。
📌 请确认是否接受方案 A
```
...
| 机制 | 适用场景 | 持久化 | 范围 |
|------|---------|--------|------|
| **Codex TaskCreate / TaskUpdate** | 会话内短任务跟踪 | ❌（会话结束即丢） | 单次会话 |
