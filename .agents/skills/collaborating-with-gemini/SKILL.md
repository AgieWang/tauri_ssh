...
```bash
# 相对路径（推荐，在项目根目录执行）
python .Codex/skills/collaborating-with-gemini/scripts/gemini_bridge.py --cd . --PROMPT "Your task"
```
...
> **推荐**：始终使用默认的沙箱模式。将 Gemini 的输出视为"脏原型"，由 Codex 重构为生产代码。
...
5. **重构代码**: 将 Gemini 的输出视为"脏原型"，由 Codex 重构为生产代码
