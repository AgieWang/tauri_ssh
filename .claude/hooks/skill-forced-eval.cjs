#!/usr/bin/env node
// Claude UserPromptSubmit Hook - 仅输出确定性命中的 Skill 与激活顺序。
// 不再维护或注入全量技能清单；路由异常时退回简短的语义评估流程。
...
路由暂不可用，请根据 \`.claude/skills/\` 的可用技能与当前任务语义选择最小完整集：
1. 列出匹配技能及简短理由；无匹配则明确说明
2. 逐个激活并完整读取所有匹配 Skill 后再执行任务
3. 根据真实文件与风险补充数据库、安全、测试和页面验收，不能因降级省略验证`;
...
const isRecoverySession = structuredRecovery || structuredStateRecovery;

if (isRecoverySession) {
  process.exit(0);
}

// 纯斜杠命令已由 Claude Code 直接展开；带参数命令仍需补充安全路由。
...
const isExpandedCommand =
  /<command-name>\s*\/[a-z0-9][a-z0-9:_-]*\s*<\/command-name>/iu.test(prompt);

if (isBareSlashCommand || isExpandedCommand) {
  process.exit(0);
}
...
function renderRoutingOutput(candidates, risks, uncertainties) {
  ...
  if (candidates.length > 0) {
    verbose.push("按以下顺序逐个激活并完整读取：");
    for (const candidate of candidates) {
      verbose.push(
        `- \`Skill(${shortText(candidate.name, 48)})\`: ${shortText(candidate.reason, 96) || "命中确定性路由规则"}`,
      );
    }
  } else {
    verbose.push(
  ...
  if (risks.length > 0) {
    verbose.push("", "风险补充：");
    for (const item of risks) verbose.push(`- ${shortText(item, 88)}`);
  }
  if (uncertainties.length > 0) {
    verbose.push("", "不确定项：");
    for (const item of uncertainties) verbose.push(`- ${shortText(item, 88)}`);
  }
  verbose.push(
    "",
    "执行要求：所有候选 Skill 激活完成后再行动；路由只缩小规则范围，真实代码、配置、数据库、风险与变更文件仍决定实现和验收。",
  );
  ...
  compact.push(
    candidates.length > 0
      ? `候选：${candidates.map((candidate) => `Skill(${candidate.name})`).join("、")}`
      : "候选：无确定命中",
  );
  if (risks.length > 0) {
    compact.push(
      `风险：${risks.map((item) => shortText(item, 40)).join("；")}`,
    );
  }
  if (uncertainties.length > 0) {
    compact.push(
      `不确定：${uncertainties.map((item) => shortText(item, 36)).join("；")}`,
    );
  }
  compact.push("要求：激活并完整读取全部候选；实际文件与风险决定验收。");
  ...
  const namesOnly = `技能:${candidates
    .map((candidate) => `Skill(${candidate.name})`)
    .join(",")}`;
...
let routing;
try {
  const { routePrompt } = require("../../.codex/scripts/skill-router.cjs");
  if (typeof routePrompt !== "function") {
    throw new TypeError("routePrompt export is unavailable");
  }
  routing = routePrompt(prompt, { mode, platform: "claude" });
} catch {
  fallback(true);
  process.exit(0);
}
