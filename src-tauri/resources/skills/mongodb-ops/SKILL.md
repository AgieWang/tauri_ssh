...
🔴 **MongoDB 的数据操作（find/aggregate/createIndex/rs.status 等）走 `ssh_exec mongosh --eval '...'`**——Tauri SSH 的 `db_*` 工具**只认 mysql/postgres/sqlite**（`detect_driver_from_kind`），**没有 MongoDB 适配**。Tauri SSH 能加密存 `mongodb_conn` 凭据供「人用数据库工作台」，但 AI 侧不能用 `db_query` 查 Mongo，别尝试。

⚠️ `mongosh --eval` 里的写操作、`sudo` 重启都会触发**用户审批**——提前告知用户，被拒后不要原样重试。

## ⭐ 装机：`install_deployment_image_store_app（镜像商店应用 "mongodb"）` 一把过（镜像商店同款，进记录）

### 装前**强制**探测（避免重复装/撞端口）
1. MCP `list_database_connections` 查现有 `mongodb_conn`——已装就复用（共享设施）。
2. 端口 27017 是否占用（`port_check`）。

🔴 **装 MongoDB 一律用 `install_deployment_image_store_app`**——mongodb 在 Tauri SSH 镜像商店目录里，`install_deployment_image_store_app` = 镜像商店 UI 同款：密码 Tauri SSH 生成并**同步进容器+安全凭证库（两边一致、必连得上）**、容器 `tauri-ssh-mongodb`、绑 `127.0.0.1`、compose 落 `/opt/tauri-ssh/stacks/mongodb`、**生成对应数据库管理连接，凭据由后端加密保存**（「数据库→MongoDB」页即装即连）。

```json
{ "tool": "install_deployment_image_store_app", "args": { "serverAlias": "<别名>", "appKey": "mongodb" } }
```
可选 `version` / `port`（默认 27017）。label 通用名、共享复用。

> ⛔ **别用 `自定义部署脚本` 手写 docker-compose 装 MongoDB**——手写易致"存的密码≠容器密码"（尤其 data 卷非空时新密码用不了），且容器命名/路径不规范。`自定义部署脚本` 只留给镜像商店目录里没有的自定义服务。
> 注：data 卷非空（装失败重来）→ 先 `rm -rf <stacks>/mongodb/data` 再重 `install_deployment_image_store_app`；复用旧数据需旧密码、用 `upsert_secure_credential` 登记。

装完会生成对应连接，用户在「数据库管理」页可直接测试和使用。**当前 Tauri SSH MCP `db_query` 暂未适配 MongoDB**（仅 mysql/postgres/sqlite），但凭据已加密入库，用户可在 mongosh 自己用；后端补上 mongo driver 后 mongodb_conn 自动联动。
...
> 💾 **备份产物统一落 `~/.tauri-ssh/backups/`**（Tauri SSH 远程工作区），别臆造 `/backup`、`/data/backup`。先 `ssh_exec mkdir -p ~/.tauri-ssh/backups`。

```bash
# 单库
mongodump --uri="mongodb://user:pass@host/mydb?authSource=admin" -o ~/.tauri-ssh/backups/mongo-$(date +%F)

# 全库
mongodump --uri="mongodb://user:pass@host/?authSource=admin" -o ~/.tauri-ssh/backups/mongo-$(date +%F)
mongodump -h host -u admin -p --authenticationDatabase admin -o ~/.tauri-ssh/backups/mongo-$(date +%F)
...
mongodump --uri="mongodb://user:pass@sec1,sec2/mydb?readPreference=secondary&authSource=admin" \
    --oplog --gzip --archive=~/.tauri-ssh/backups/dump-$(date +%F).gz
